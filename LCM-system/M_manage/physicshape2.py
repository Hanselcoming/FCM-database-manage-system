...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.gridLayout.addWidget(self.inxz, 1, 4, 1, 1)
        ...
        self.gridLayout.addWidget(self.in50, 2, 4, 1, 1)
        ...
        self.gridLayout.addWidget(self.incjb, 3, 4, 1, 1)
    ...
    def query(self):
        ...
        result = service.query("select * from 物理形状表")  # 调用a服务类中的公共方法执行查询语句
    ...
    def getName(self, name):
        result = service.query("select * from 物理形状表 where 编号= %s", name)
        return len(result)
            ...
            if inbh != "":  # 判断编号不为空
                ...
                #     QMessageBox.information(None, '提示', '您要添加的材料必须先在基本信息表中添加编号！', QMessageBox.Ok)
                    ...
                    if service.getName(inbh) == 0:
                        ...
                        # if (type(float(in50)) == float or in50 == '') and (
                        #         type(float(in90)) == float or in90 == '') and (
                        #         type(float(incjb)) == float or incjb == ''):
                        service.exec2("set sql_mode='' ")

                        result = service.exec(
                            "insert into 物理形状表(编号,形状,密度,D50,D90,长径比) values (%s,%s,%s,%s,%s,%s)",
                            (inbh, inxz, inmd, in50, in90, incjb))
        ...
        # try:
            ...
            if inbh != "":  # 判断编号不为空
                if self.getName(inbh) <= 0:
                    ...
                    service.exec2("set sql_mode='' ")
                    result = service.exec(
                        "update 物理形状表 set 形状=%s,密度=%s,D50=%s,D90=%s,长径比=%s where 编号=%s",
                        (inxz, inmd, in50, in90, incjb, inbh))
        ...
        try:
            ...
            if inbh != "":
                if self.getName(inbh) <= 0:  # 判断编号是否存在
                    ...
                    result = service.exec("delete from 物理形状表 where 编号= %s", (inbh,))
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "材料按物理形状管理"))
        ...
        self.label_4.setText(
            _translate("MainWindow", "<html><head/><body><p align=\"center\">密度</p></body></html>"))
        self.label.setText(_translate("MainWindow", "(g/cm^3)"))
        self.label_5.setText(
            _translate("MainWindow", "<html><head/><body><p align=\"center\">D50</p></body></html>"))
        self.label_8.setText(_translate("MainWindow", "(μm)"))
        self.label_6.setText(
            _translate("MainWindow", "<html><head/><body><p align=\"center\">D90(μm)</p></body></html>"))
        self.label_9.setText(_translate("MainWindow", "(μm)"))
        ...
        item.setText(_translate("MainWindow", "密度"))
        item = self.tableWidget.horizontalHeaderItem(3)
        item.setText(_translate("MainWindow", "D50"))
