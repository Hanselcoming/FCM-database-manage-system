...
class Ui_MainWindow(QMainWindow):
    ...
    def query(self):
        ...
        result = service.query("select * from 抗压能力表")  # 调用服务类中的公共方法执行查询语句
    ...
    def getItem(self, item):
        if item.column() == 0:
            ...
            # 获取单击的单元格文本
            self.inyhyd.setText(item.text())  # 显示在D90降级文本框中
        elif item.column() == 3:
            self.injb.setText(item.text())  # 显示在级别文本框中

    def getName(self, name):
        result = service.query("select * from 抗压能力表 where 编号= %s", name)
        return len(result)
    ...
    def add(self):
        try:
            ...
            inyhyd = self.inyhyd.text()  # 记录修改的D90降级
            ...
            if inbh != "":  # 判断编号不为空
                if self.getName(inbh) > 0:
                    ...
                    if service.getName(inbh) == 0:
                        ...
                        result = service.exec("insert into 抗压能力表(编号,D50降级,D90降级,级别) values (%s,%s,%s,%s)",
                                              (inbh, incsyd, inyhyd,injb))
        ...
        try:
            ...
            if inbh != "":  # 判断编号不为空
                if self.getName(inbh) <= 0:
                    ...
                    result = service.exec("update 抗压能力表 set D50降级= %s,D90降级=%s,级别=%s where 编号=%s",
                                          (incsyd, inyhyd,injb, inbh))
            ...
            QMessageBox.warning(None, '警告', '请先点击编号或输入编号选择要修改更新的数据！fff', QMessageBox.Ok)
        ...
        try:
            ...
            if inbh != "":
                if self.getName(inbh) <= 0:  # 判断编号是否存在
                    ...
                    result = service.exec("delete from 抗压能力表 where 编号= %s", (inbh,))
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "材料按抗压能力管理"))
        self.label_2.setText(_translate("MainWindow",
                                        "<html><head/><body><p align=\"center\"><span style=\" font-size:10pt;\">编号</span></p></body></html>"))
        self.label_3.setText(
            _translate("MainWindow", "<html><head/><body><p align=\"center\">D50降级（%）</p></body></html>"))
        self.label_4.setText(
            _translate("MainWindow", "<html><head/><body><p align=\"center\">D90降级（%）</p></body></html>"))
