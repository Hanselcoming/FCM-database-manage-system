...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.gridLayout.addWidget(self.injb, 4, 5, 1, 3)
    ...
    def new(self):
        self.inbh.setText("")
        self.injb.setText("")
        self.incsmin.setText("")
        self.incsmax.setText("")
        self.inyhmin.setText("")
        self.inyhmax.setText("")
        self.inmhmin.setText("")
        self.inmhmax.setText("")
        self.tableWidget.setRowCount(0)  # 清空表格中的所有行
        result = service.query2("select * from v_sph")  # 调用服务类中的公共方法执行查询语句
    ...
    def Query(self):
        try:
            ...
            incsmin = self.incsmin.text()  # 记录输入的cs最小值
            incsmax = self.incsmax.text()  # 记录输入的初始圆球度最大值
            inyhmin = self.inyhmin.text()  # 记录输入的yh最小值
            inyhmax = self.inyhmax.text()  # 记录输入的压后圆球度最大值
            inmhmin = self.inmhmin.text()  # 记录输入的mh最小值
            inmhmax = self.inmhmax.text()  # 记录输入的磨后圆球度最大值
            injb = self.injb.text()  # 记录输入的名称



            # 获取所有材料基本信息
            if (inbh == "") and (injb == "") and (incsmin == "") and (incsmax == "") and (inyhmax == "") and (
                    inyhmin == "") and (
                    inmhmin == "") and (inmhmax == ""):
                self.new()  # 在表格中显示最新数据
            ...
            #     QMessageBox.warning(None, '警告', '请检查所要查询的数据最小最大值！', QMessageBox.Ok)
                ...
                result1 = service.choose(incsmin, incsmax, '圆球度表', '初始圆度')  # 按照初始圆球度查询
                a = service.exec2("alter table t_temp rename to t_temp_1")
                if a>0:
                    result2 = service.choose(inyhmin, inyhmax, 't_temp_1', '压后圆度')  # 按照压后圆球度查询
                    b=service.exec2("alter table t_temp rename to t_temp_2")
                    if b>0:
                        result3 = service.choose(inmhmin, inmhmax, 't_temp_2', '磨后圆度')  # 按照磨后圆球度查询
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "按圆球度查询"))
        self.label.setText(_translate("MainWindow",
                                      "<html><head/><body><p align=\"center\"><span style=\" font-size:18pt;\">按圆球度查询</span></p></body></html>"))
